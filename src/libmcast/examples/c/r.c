...
unsigned short gport[2] = {1025, 1026};
...
main(int argc, char *argv[]) {
  ...
  int sock[2];                 
  int flag_on = 1;              
  struct sockaddr_in mc_addr[2];
  char recv_str[MAX_LEN+1];
  int recv_len;            
  struct ip_mreq mc_req[2];
  char* mc_addr_str[2];    
  ...
  mc_port = gport[0];		/* same port per ip */

  for(i = 0; i < N; ++i){
    mc_addr_str[i] = gip[i];

#if 0
    mc_port = gport[i];		/* unique port per ip */
#endif
    ...
    if ((setsockopt(sock[i], SOL_SOCKET, SO_REUSEPORT, &flag_on,
#else
   ...
   /*    mc_addr[i].sin_addr.s_addr = htonl(INADDR_ANY); */
		    mc_addr[i].sin_addr.s_addr = inet_addr(mc_addr_str[i]);
