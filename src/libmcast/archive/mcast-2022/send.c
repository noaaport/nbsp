...
#include <time.h> /* only to send the time() (in the test data) */
#include "mcast.h"
...
#define MAXLINE 1024
  ...
  snprintf(buf, MAXLINE + 1, "%s: %d %lu", name.nodename, getpid(), time(NULL));
  fprintf(stdout, "%s\n", buf);
  n = sendto(s, buf, strlen(buf), 0, sa, sa_len);
  if(n == -1)
    err(1, "sendto");
