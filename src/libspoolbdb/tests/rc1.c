...
/*
 * Same as wrc, but reading only from a cache-only db produced by wrc1.
 *
 ...
 * ./wrc1 in/*
 *
 * Then in separate terminal run ./rc1.
 *
 * The files stored in the spoolbdb, are then read and written in the
 * current directory.
 */
...
int spool_open(char *dbhome, int dbenv_mode, uint32_t dbcache_mb,
  ...
  /* The last argument sets f_rdonly = 1 */
  status = spoolbdbv_open(&gdbv, dbhome, dbenv_mode, dbcache_mb,
			 Ndb, nmax, dbfname, dbfile_mode, f_mpool_nofile, 1);
