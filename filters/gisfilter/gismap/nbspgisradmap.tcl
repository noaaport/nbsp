...
# $Id$
# 
# Usage: nbspgisradmap [-b] [-k] [-d <outputdir>] [-D <defines>]
#                  [-e <extent>] [-f <mapfonts_dir>] [-g <geodata_dir>]
#                  [-I <inputdir>] [-m | -M <map_template>] [-n <index>]
...
foreach inputfile $nbspgismap(input_files_list) {
    ...
    if {$option(v) == 1} {
	puts "Converting $inputfile to shapefile.";
    }
    if {$do_nbspunz == 1} {
